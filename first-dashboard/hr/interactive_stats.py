...
from hr.hr_data import hr_employee_named_skill, skill_series
...
def interactive_stats_page(): 
    return ui.page_sidebar(
        ui.sidebar(
            ui.h2("Informations"),
            ui.input_select("skill_name", "choisissez une compétence", skill_series.to_series().to_list())
        ),
...
# TOTAL EMPLOYEE COUNT
    def getTotalEmployeeCount():
        return f"{hr_employee_named_skill.select("employee_id").count().get_column("employee_id")[0]}"
...
# AVG OF PEOPLE WITH A SKILL
    ...
    def getCountPeopleWithSkill():
        ...
        ppl_with_skill = hr_employee_named_skill.filter(
            pl.col("name") == input.skill_name()
        ).count().get_column("employee_id")[0]

        print(hr_employee_named_skill.group_by("name").all())
