...
def getSharedData():
    ...
    # tables
    shared_data.res_company = cx.read_sql(
        conn=connection, query="SELECT * FROM res_company", return_type="polars"
    )
    shared_data.res_partner = cx.read_sql(
        conn=connection, query="SELECT * FROM res_partner", return_type="polars"
    )
    ...
    for i in range(len(shared_data.company_id_dict) + 1):
        ...
        shared_data.company_dict[
            f"{shared_data.company_id_dict['name'][i]}".replace(" ", "").strip()
        ] = shared_data.company_id_dict["id"][i]
