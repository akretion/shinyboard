...
def suppliers_stats_page():
    return ui.page_sidebar(
        ui.sidebar(
            ...
            ui.input_select(
                "company_name",
                ui.h4("Sélectionnez une entreprise"),
                shared_data.company_names,
            ),
        ),
        ui.card(
            ui.card_header("Commandes en retard"),
            ui.card_body(ui.output_data_frame("displayOverdueOrders")),
        ),
        ui.h2("Achats"),
        ui.card(
            ui.card_header("Commandes"), ui.card_body(ui.h3("pas encore implémenté."))
        ),
        ui.card(
            ui.card_header("Fournisseurs"),
            ui.card_body(ui.output_data_frame("displaySuppliers")),
        ),
    )
...
def suppliers_stats_server(input, output, session):
    ...
    def displaySuppliers():
        return (
            purchase_data.suppliers.filter(
                # input.dateslider()[0] <= pl.col("write_date"),
                # input.dateslider()[1] >= pl.col("write_date"),
                shared_data.company_dict[
                    f"{input.company_name()}".replace(" ", "").strip()
                ]
                == pl.col("company_id")
            )
            .select("name", "email", "website")
            .rename({"name": "nom du fournisseur", "website": "site web"})
        )

    @render.data_frame
    def displayOverdueOrders():
        print(
            shared_data.company_dict[f"{input.company_name()}".replace(" ", "").strip()]
        )
        return (
            shared_data.res_partner.rename(
                {
                    "id": "res_partner_id",
            ...
            .join_where(
                purchase_data.purchase_order.rename({"id": "id de la commande"}).filter(
                    shared_data.company_dict[
                        f"{input.company_name()}".replace(" ", "").strip()
                    ]
                    == pl.col("company_id")
            ...
            .filter(pl.col("receipt_status").is_null())
            .with_columns(date=datetime.now())
            .rename({"date": "date actuelle"})
            .with_columns(
                (pl.col("date_planned") - pl.col("date actuelle"))
